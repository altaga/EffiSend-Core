...
const getBalanceTokens = tool(
  ...
  {
    name: "get_balance_core",
    description: `This tool retrieves the user's current X token balance, where X is any token on Core Mainnet. Use this when the user specifically asks for their token balance, 'token', "balance", or general wallet funds on Core Mainnet.
...
const transferTokens = tool(
  async ({ amount, to, token }, { configurable: { user } }) => {
    ...
    const { hash } = response;
    return JSON.stringify({
      message: "Transaction created and available on Core Mainnet.",
      status: "success",
      transaction: hash,
    });
  },
  {
    ...
    description: `This tool facilitates Tokens transfers on the Core Mainnet. It generates the transaction data for the user to sign. It activates whenever the user explicitly requests to send Tokens, initiates a transaction, or mentions terms like 'transfer,' 'Tokens,' or 'Core Mainnet' in relation to their wallet activity.
      `,
    schema: z.object({
      amount: z.string(),
      to: z.string(),
...
// Transfer Core USDC to Linea USDC (MetaMask Card) - Modified to return transaction data to API
...
const listOfTools = tool(
  () => {
    ...
    return JSON.stringify({
      status: "info",
      message:
        "DeSmond can help you fund your MetaMask card, transfer tokens on Core Mainnet, and retrieve your current balance for any token on Core Mainnet.",
    });
  },
  {
    name: "list_of_tools",
